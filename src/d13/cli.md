...
Cisco IOS Catalyst 交换机上的多个命令行接口 (CLI) 命令可用于排除第 1 层问题。常用的命令包括 show interfaces、show controllers 和 show interface [name] counters errors 命令。除了掌握这些命令外，您还需要能够准确解释这些命令提供的输出或信息。show interfaces 命令是一个功能强大的故障排除工具，可提供大量信息，包括以下内容：
交换端口的管理状态
端口运行状态
介质类型（针对选定的交换机和端口）
端口输入和输出数据包
...
如同将在生成树协议（Spanning Tree Protocol, STP）章节中介绍的那样，全双工不匹配可能导致在某端口连接到另一交换机时，交换网络中的 STP 循环。这些不匹配**可通过手动配置交换机端口的速率和双工方式予以解决。**
