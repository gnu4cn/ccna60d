...
常见广域网协议包括PPP、HDLC和帧中继。当然还有其他多种协议，但您需重点关注CCNA课程大纲涵盖的内容。点对点协议（PPP）适用于思科设备与非思科设备互联的场景，其优势在于包含身份验证功能。 该协议支持多种连接类型，包括DSL、电路交换及异步/同步连接。思科的高级数据链路控制协议（cHDLC）是其对开放标准版HDLC的实现。HDLC要求设备分为数据终端设备（DTE）和数据通信设备（DCE），并作为思科路由器（串行接口）的默认封装类型。DCE会发送保持活动信号以检测链路状态。
