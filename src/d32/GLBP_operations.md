...
**图 32.26** -— **GLBP 配置示例的拓扑结构**

**注意**：这里假设了这些交换机之间的 VLAN 及中继配置已就位，同时这些交换机能成功通过 `VLAN 192` 互相 `ping` 通。出于简洁目的，这些配置输出，将在这些配置示例中省略。
...
一旦这个 GLBP 分组得以配置，那么 `show glbp brief` 这条命令，便可用于查看这一 GLBP 配置摘要信息，如下所示中输出：
...
从上面的输出中，我们根据 `VTP-Server-1`（`192.168.1.1`） 高于全部其他网关的 110 的优先级值，确定其已被选为 AVG。网关 `VTP-Server-4`（`192.168.1.4`）已被选为备用虚拟网关，因为他有着剩余三个网关中最高的 IP 地址，尽管他们都共用了同一优先级值。因此，网关 `VTP-Server-2` 和 `VTP-Server-3` 就被置于监听状态。

`show glbp` 这条命令，打印了这个 GLBP 分组状态的详细信息。这一命令的输出如下所示：
...
在 AVG 上执行 `show glbp` 这条命令时，除其他信息外，他还会显示备用虚拟网关的地址，与那些该分组中 AVF 的编号，以及指派给他们的状态。每个 AVF 的虚拟 MAC 地址也会得以显示。
