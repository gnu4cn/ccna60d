...
虚拟路由器冗余协议（VRRP）是一种网关选举协议，可动态地将一个或多个虚拟网关的责任分配给局域网上的 VRRP 路由器，从而允许以太网等多接入网段上的多个路由器使用相同的虚拟 IP 地址作为默认网关。
VRRP 的运行方式与 HSRP 类似；但与 HSRP 不同的是，VRRP 是一种开放标准，由 RFC 2338 定义，RFC 3768 已将其淘汰。VRRP 使用 112 号 IP 协议向组播目标地址 224.0.0.18 (VRRP)发送广告。在数据链路层，广告从主虚拟路由器 MAC 地址 00-00-5e-00-01xx 发送，其中 "xx "代表两位十六进制组号。图 32.18 举例说明了这一点。
