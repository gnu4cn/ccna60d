...
默认情况下，HSRP 信息使用纯文本密钥字符串 "cisco "发送，这是验证 HSRP 对等体的一种简单方法。如果信息中的密钥字符串与 HSRP 对等设备上配置的密钥相匹配，则信息被接受。如果不匹配，HSRP 会忽略未验证的报文。纯文本密钥的安全性很低，因为使用简单的数据包捕获工具（如 Wireshark 和 Ethereal）就可以 "在网络上捕获 "它们。下图 32.13 显示了 HSRP 信息中使用的默认纯文本验证密钥。
