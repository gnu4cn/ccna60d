...
一个 EtherChannel，由物理的、单条的一些 `FastEthernet`、`GigabitEthernet` 或 `Ten-GigabitEthernet`（10Gbps）链路组成，这些链路捆绑在一起成为一个逻辑链路，如下图 11.1 所示。由快速以太网链路组成的 EtherChannel 称为快速以太网通道 (FEC)；由千兆以太网链路组成的 EtherChannel 称为千兆以太网通道 (GEC)；最后，由万兆以太网链路组成的 EtherChannel 称为万兆以太网通道 (10GEC)。每个 EtherChannel 最多可由八个端口组成。EtherChannel 中的物理链路必须具有相似的特性，例如，定义在同一 VLAN 中，或具有相同的速度和双工设置。在 Cisco Catalyst 交换机上配置 EtherChannels 时，一定要记住不同型号的 Catalyst 交换机支持的 EtherChannels 数量会有所不同。例如，在 Catalyst 3750 系列交换机上，范围是 1 到 48；在 Catalyst 4500 系列交换机上，范围是 1 到 64；而在旗舰版 Catalyst 6500 系列交换机上，EtherChannel 配置的有效值数量取决于软件版本。对于 12.1(3a)E3 版之前的版本，有效值范围为 1 至 256；对于 12.1(3a)E3 版、12.1(3a)E4 版和 12.1(4)E1 版，有效值范围为 1 至 64。12.1(5¢)EX 及以后版本最多支持 64 个值，范围从 1 到 256。
