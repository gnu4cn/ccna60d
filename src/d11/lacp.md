...
链路聚合控制协议 (LACP) 是 IEEE 802.3ad 规范的一部分，用于从多个物理链路创建逻辑链路。由于 LACP 和 PAgP 不兼容，因此链路的两端都需要运行 LACP，以便自动组建 EtherChannel 组。与 PAgP 一样，在配置 LACP EtherChannels 时，所有 LAN 端口必须具有相同的速度，并且必须配置为第 2 层或第 3 层 LAN 端口。如果端口通道内的一条链路发生故障，之前通过故障链路传输的流量将被切换到端口通道内的其余链路。此外，当您更改端口通道中活动捆绑端口的数量时，流量模式将反映端口通道的重新平衡状态。LACP 支持通过在端口间交换 LACP 数据包自动创建端口通道。它动态学习端口组的功能，并通知其他端口。一旦 LACP 识别出匹配正确的以太网链路，它就会帮助将链路分组为千兆以太网端口通道。与要求端口具有相同速度和双工设置的 PAgP 不同，LACP 规定端口只能是全双工的，因为不支持半双工。LACP EtherChannel 中的半双工端口将处于暂停状态。默认情况下，端口通道中一条链路上的所有入站广播和组播数据包都会被阻止返回端口通道的任何其他链路。LACP 数据包会发送到 IEEE 802.3 Slow Protocols Multicast 组地址 01-80-C2-00-00-02。LACP 帧使用 EtherType 值 0x8809 编码。下图 11.4 展示了以太网帧中的这些字段。
