...
对于 PAgP 和 LACP 以太通道，Catalyst 交换机使用多态算法，利用数据包头部的关键字段生成哈希值，然后将其与以太通道组中的物理链路匹配。换句话说，交换机通过将二进制模式的一部分（由帧中的地址形成）还原为一个数值，从而选择 EtherChannel 中的一条链路，从而在 EtherChannel 的链路间分配流量负载。这一操作可以在 MAC 地址或 IP 地址上执行，也可以仅基于源地址或目标地址，甚至同时基于源地址和目标地址。虽然详细研究 EtherChannel 负载分配中使用的哈希值的实际计算方法超出了 CCNA 考试要求的范围，但重要的是要知道管理员可以定义报头中的哪些字段可用作用于确定数据包物理链路传输的算法的输入。负载分配方法通过端口通道负载平衡 [method] 全局配置命令进行配置。任何时候只能使用一种方法。下表 11.3 列出并描述了 Cisco IOS Catalyst 交换机在配置 EtherChannel 负载分配时可用的不同方法：
表 11.3-以太通道负载分配（负载平衡）选项
