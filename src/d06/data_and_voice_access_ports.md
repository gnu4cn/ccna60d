...
所谓 IP 电话，是指一种使用 IP 数据包，发送及接收语音流量的特殊类型电话。话机会连入交换机，就像笔记本电脑和台式机一样。然后，话机会像大楼中的全部其他联网设备一样，使用同一网络通信。他们不再需要与 PBX 通信以控制呼叫；而是与位于某台服务器上的应用通信。若咱们对这方面感兴趣，那么可在 [CCNA 协作考试](https://www.cisco.com/site/us/en/learn/training-certifications/certifications/collaboration/index.html) 中，了解更多有关语音和视频网络的知识。
...
> **译注**：这段话的意思是，即使我们在配置这个端口时从未涉及 `trunk` 关键字，而只是在该端口上配置了两个 `access` 与 `voice` VLAN，该端口仍会以 `trunk` 模式运行。
