...
当通过交换机的某次 `ping` 不成功时，那么就要检查确保终端设备是在同一 VLAN 中。每个 VLAN 都被视为一个网络，而出于这一原因，因此其就必须有着与其他任何 VLAN 不同的地址范围。为了让一个 VLAN 与另一 VLAN 相连，一台路由器就必须要路由流量。

### 接口问题


默认情况下，所有路由器接口都是对流量关闭的，而交换机接口都是开放的。当咱们发现咱们交换机已将其接口管理性关闭时，那么就要打开接口，及接口必须以 `no shut` 这个接口级命令设置。
...
二层接口可被设置为三种模式：中继、接入或动态。中继模式允许交换机连接到另一交换机或服务器；而接入模式用于终端设备，如 PC 或笔记本电脑；动态模式让交换机检测要选取的设置。


在诸如 3550 型号交换机等平台上，默认模式通常是动态 `desirable`，但请在 Cisco.com 上查看咱们型号设置与发布说明。对于 CCNA 考试，咱们将被要求配置 2960 型交换机。他将动态地选择模式，除非咱们将其硬性设置为中继或接入模式。
...
交换机端口的默认设置，为自动检测的双工及自动检测的速率。当咱们将某个 10Mbps 设备，插入运行于半双工的某台交换机（若咱们能找到这样的设备）时，那么该端口应检测到这点并正常工作。但情况并非总是如此，因此一般建议是要硬性设置该交换机端口的速率和双工，如下输出中所示。
...
双工不匹配的一些迹象（除错误消息外），包括接口上的输入与 CRC 报错，如下输出中所示。另请参阅一层和二层故障排除小节。
...
与任何电气设备一样，交换机上的端口也会失效，或仅在部分时间内工作，这就会更难故障排除。工程师通常会通过将某个已知工作的设备，插入交换机上的另一端口来测试该接口。咱们也可重启端口，即先应用 `shut` 命令，然后再应用 `no shut` 命令。交换网线也是一种常见的故障排除步骤。其他常见的交换机故障与解决方案，如下图 6.20 中所示。

请查看咱们交换机的文档，因为除系统和端口的 LED 指示灯外，每个端口还会显示闪烁或常亮的红色、琥珀色及绿色，分别表示功能正常或端口/系统问题。
