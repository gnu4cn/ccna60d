...
交换机存在即是为交换数据帧（即把来自某个接收接口的数据帧，传送到正确输出接口）。广播数据帧和带有未知目的地（不在 MAC 数据表中）的数据帧，会被交换出除他们被接收接口外的全部接口（称为数据帧泛洪）。为了实现这一功能，交换机会执行三种操作：

- 根据目标 MAC 地址，转发（交换）或过滤（丢弃）数据帧
- 从进入数据帧学习 MAC 地址
- 使用 STP 防止二层环路（STP 将在稍后介绍）

在下图 6.6 中，交换机过滤了离开接口 `FO/2` 的数据帧，并在该数据帧源自主机 A 目的为主机 C 时，将其正确转发出 `F0/3`：
