...
众所周知，距离矢量路由协议因其简单的 "谣言路由 "方法而容易出现重大问题。距离矢量协议和链路状态协议使用不同的技术来避免路由问题。最重要的机制包括以下几种（其中一些我们已简要提及）：无效计时器：当长时间未收到路由更新时，这些计时器将路由标记为不可达。跳数限制：当路由的跳数超过预定义的跳数时，该参数会将路由标记为不可达。RIP 的跳数限制为 15，因为它通常不用于大型网络。不可达路由不会作为最佳路由安装到路由表中。跳数限制可以防止更新在网络中循环，就像 IP 头中的 TTL 字段一样。触发更新：此功能允许在重要更新的情况下绕过更新计时器。例如，如果必须通过网络传播重要的路由更新，RIP 30 秒计时器可以被忽略。保留计时器：如果某个路由的度量值不断变差，则在一段延迟时间内不接受该路由的更新。异步更新：异步更新是另一种安全机制，可防止路由器在同一时间泛洪整个路由信息。如前所述，OSPF 每 30 分钟更新一次。异步更新机制会为每个设备产生一个小延迟，这样它们就不会在同一时间泛洪信息。这提高了带宽利用率和处理能力。路由中毒：该功能可防止路由器通过已失效的路由发送数据包。距离矢量协议用它来表示路由已无法到达。这是通过将路由度量设置为最大值来实现的。分割水平线：分割地平线可防止更新从同一接口发送出去，因为该区域的路由器应该已经知道该特定更新。中毒反向：这种机制是中毒路由的分裂水平规则的例外情况。
### 路由故障避开机制
