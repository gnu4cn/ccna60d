...
路由协议有两大类：
距离矢量
距离矢量路由协议传统上使用一维矢量来确定通过网络的最优路径，而链路状态路由协议则使用最短路径优先（SPF）来确定通过网络的最优路径。在深入研究这两类路由协议的具体内容之前，我们首先来了解一下向量以及难以捉摸的 SPF 算法。了解向量 一维向量是一个有向的量。它只是一个特定方向或路线上的量（数字）。向量的概念如下图 17.11 所示。
