...
路由算法设计得尽可能简单。除了提供支持复杂互联网部署的能力外，路由协议还应考虑到运行该算法所需的资源。与其他算法相比，一些路由算法需要更多硬件或软件资源（如 CPU 及内存）运行；但相比一些替代的简单算法，他们具备提供更多功能的能力。


## 灵活性

除提供路由功能外，路由算法还应特性充沛，允许他们支持不同网络中遇到的不同需求。需要注意的是，这种通常会以一些别的特性为代价，例如接下来介绍的收敛性。
