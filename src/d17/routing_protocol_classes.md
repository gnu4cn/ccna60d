...
所谓一维矢量值，是个有向的量。他只是个某个特定方向，或路线上的量（数字）。矢量这一概念，在下图 17.11 中得以演示。

![理解矢量](../images/1011.png)

**图 17.11** -- **理解矢量值**

参照图 17.11，第一行于 0 处开始并在 8 处结束，而第二行则于 8 处并在 13 处结束。第一行的矢量值为 8，而第二行的矢量值为 5。运用基本数学知识，我们知道 8 + 5 = 13。向量的起点和终点并不重要。实际上，唯一重要的是矢量有多长，以及矢量移动了多远。
...
利用计数到无穷大这一特征，当某个目的网络，远于该路由协议所允许的最大跳数时，那么该网络就会被视为不可达。该网络条目因此便不会安装到 IP 路由表中。

水平分割则规定了，路由信息不能通过其被接收的同一接口，再发送出去。这一特征阻止了信息重新通告回该信息被学习到的来源。虽然这一特征是一种很好的循环预防机制，但其也是个重大缺陷，尤其是在中心辐射型网络中。

反向投毒（或路由中毒）是对水平分割的扩展。当与水平分割结合使用时，反向投毒允许网络从他们被接收的同一接口在通告出去。不过，反向投毒会导致路由器将这些网络，以一个 “不可达” 的度量值通告回发送他们的路由器，从而接收到这些条目的路由器，就不会将他们添加回其路由表中。
...
两种最常见的距离矢量路由协议，分别是 RIP 及 IGRP。EIGRP 属于一种先进的距离矢量路由协议，同时使用了距离矢量余链路状态的一些特性（即，他时一种混合协议）。


### 链路状态的路由协议

链路状态路由协议是一种分层路由协议，它使用区域概念对网络中的路由器进行逻辑分组。与距离矢量路由协议相比，链路状态路由协议的扩展性更好，运行效率更高。运行链路状态路由协议的路由器会创建一个包含完整网络拓扑结构的数据库。这样，同一区域内的所有路由器都能拥有相同的网络视图。
由于网络中的所有路由器都拥有相同的网络视图，因此在网络间转发数据包时会使用最优路径，从而消除了路由环路的可能性。因此，"分裂地平线 "和 "路由中毒 "等技术并不适用于链路状态路由协议，因为它们不适用于距离矢量路由协议。
链路状态路由协议通过向同一区域内的所有其他路由器发送链路状态广告或链路状态数据包来运行。这些数据包包括连接接口、指标和其他变量的信息。路由器积累这些信息后，就会运行 SPF 算法，计算出通往每个路由器和目标网络的最短（最佳）路径。路由器利用接收到的链路状态信息建立链路状态数据库（LSDB）。当两个相邻路由器的 LSDB 同步时，这两个路由器就被称为相邻路由器。
