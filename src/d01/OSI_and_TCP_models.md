...
另一方面，UDP 被认为是种无连接协议。他会编号每个数据包，然后将他们发送到他们的目的地。他从不检查这些数据包是否安全到达，也不会在发送数据包前建立连接。有时，数据并不那么重要，应用开发者会决定，当信息未能到达其目的地，其总是可被再次发送。
...
在这层，数据帧被转换成放置在线路上的比特位。这些比特位由电脉冲组成，分别读作 “开” 和 “关” 位，或二进制的 `1s` 和 `0s`。集线器工作在这一层，同时在这里咱们将发现一些诸如 RJ45 的电缆规格。
...
**第 3 层** -- 是否使用了正确的路由协议，路由器是否正通告着正确的网络？
