...
在定义他们要保护以免受威胁的那些网络区域时，防火墙会用到安全区的概念。防火墙通过具体定义哪个网络主机可发起从一个安全区到另一安全区连接的规则，提供保护。这些规则被编程到应用中，并保存在防火墙上。一般来说，受保护的网络越小，规则就越少，而该网络越大，则所需的规则就越多。对于一些大型企业网络，防火墙实际上可能有成百上千的规则，管理起来可能非常复杂。
...
安全区可根据数据库、Web 服务器、机密信息，甚至根据那些需要受限进出访问区域的人员团队创建。无论何种类型的安全区创建，防火墙都是其理想方案。


## 防火墙的高级功能


如果防火墙的功能仅限于控制安全区域间的流量放行或拦截，那么它们很容易被配置了 ACL 或前缀列表的路由器所取代。防火墙真正的价值在于：作为专门的安全设备，它们具备特定的软件、处理能力和独立的网络架构，能够处理路由器所不擅长的复杂任务。通过将这些安全任务交给防火墙，路由器便能专注于其核心设计目标——高效地转发数据包。
