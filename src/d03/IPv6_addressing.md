...
所谓 IPv6 的全局前缀，用于表示 IPv6 的全局地址空间。全部 IPv6 的全球互联网地址，都位于 `2000::/16` 到 `3FFF::/16` 的范围内。而 16 位的运营商持有 IPv6 前缀，属于分配给运营商并由提供商持有的。这些前缀的分配，遵循了与 IPv4 下前缀分配相同的规则。运营商持有的前缀，位于 `2000:0000::/32` 到 `3FFF:FFFF::/32` 范围内。

接下来的 16 位，表示由具体运营商，从提供商分配的前缀地址空间，分配给某个组织的 IPv6 前缀。这个前缀位于 `[前32位]:0000::/48` 到 `[前32位]:FFFF::/48` 范围内。总的来说，这前 48 位，被称为运营商分配的前缀，如下图 3.1 所示。
...
所谓站点前缀，是 48 位运营商分配前缀后接下来的 16 位。站点前缀的子网掩码长度为 `/64`，其包括了 48 位运营商分配的前缀。这一前缀长度，又允许每个站点前缀包含 $2^{64}$  个地址。下图 3.2 展示了这个 16 位的站点前缀。
