...
- 链路本地地址
- 站点本地地址（译注：已废除，取而代之的是本地唯一地址，Unique-Local addresses, ULAs）
- 聚合全球单播地址
- 组播地址
- 任意播地址
- 环回地址
...
虽然对 IPv6 执行 NAT 是可行的，但其不被建议；这正是 IPv6 地址大得多的原因。站点本地地址由 `FECO::/10` 前缀、54 位的子网 ID，及由链路本地地址用到的 EUI-64 格式接口标识符组成。尽管链路本地地址中的那 54 位都被设置为 `0` 的值，站点本地地址中的同样 54 位，会被用于创建不同的 IPv6 前缀（最多 $2^{54}$54 个）。站点本地地址的格式如下图 3.7 中所示。
...
所谓唯一本地地址，是分配自 `FC00::/7` 这个 IPv6 地址块，其随后被进一步分为两个 `/8` 的地址组，分别称为分配组与随机组。这两个组分别是 `FC00::/8` 与 `FD00::/8` 两个 IPv6 地址块。`FC00::/8` 区块由专门的分配机构负责管理, 旨在向用户发放其所需的 /48 地址空间，而 `FDO00::/8` 这个地址块，则是通过追加一个随机生成的 40 位字符串形成，以派生出一个有效的 `/48` 地址块。
...
截止这一教学模组编写时，`2000::/3` 这个 IPv6 地址块中，只有三个子网已被分配使用。这些分配如下表 3.5 中所示。
...
这一小节前面引入的任意播，可简单地描述为一对最近的通信，one-to-nearest communication，因为根据路由协议度量值，那个离得最近的共用地址，将总是会被本地设备优先选用。在 IPv6 下，没有为任意播专门分配的范围，因为任意播地址会使用全局单播、站点本地甚至链路本地地址。不过，有种任意播地址被保留作专门用途。这种特殊地址被称为 `Subnet-Router` 任播地址，由子网的 64 位单播前缀，与其余 64 位设置为零所形成（例如，`2001:1a2b:1111:d7e5::`）。任播地址不得用作 IPv6 数据包的源地址。这些地址通常会被诸如移动 IPv6 等协议用到，而这已超出 CCNA 的范围。

## 环回地址

IPv6 下的环回地址，会以与 IPv4 中的相同方式被用到。每个设备都有个 IPv6 回环地址，相当于 IPv4 下用到的 `127.0.0.1` 回环地址，这一地址由设备本身使用。IPv6 的环回地址使用前缀 `::1`，其可以首选地址格式表示为 `0000:0000:0000:0000:0000:0000:0000:0000:0001`。这意味着，在环回地址中，除总是被设置为 `1` 的最后一位外，所有的位都设置为 `0`。当 IPv6 于某一设备上启用时，这些地址总是自动分配，且他们永不能更改。

## 未指明地址

在 IPv6 分址下，所谓未指明地址，就是未分配给任何接口的一些单播地址。这些地址表明 IPv6 地址缺失，而用于包括 IPv6 的 DHCP 和 DAD 等一些特殊目的。未指明地址由 IPv6 地址中的全 `0` 值表示，可使用 `::` 前缀书写。在首选格式下，这些地址表示为 `0000:0000:0000:0000:0000:0000:0000:0000:0000:0000:0000`。
