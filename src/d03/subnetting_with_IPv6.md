...
这个前缀为 56 位，转换为 14 个十六进制数（14 x 4 = 56），因此咱们知道，这个前缀会将咱们带到某个四元组的中间。这便是考试中，咱们可能出错的地方。咱们必须将前缀中断的那个四元组中的第 3 和第 4 个十六进制位清零：
...
咱们可以借用主机部分的位用于子网，但绝对没有理由这样做，这会导致 IPv6 设计之初旨在实现的许多特性无法使用, 其中包括无状态自动配置。
