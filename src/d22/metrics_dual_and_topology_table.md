...
在部署 EIGRP 时，重要的是要掌握，路由被实际放入 IP 路由数据表前，在协议内部及由协议用到的各个方面非常重要。在这一小节中，咱们将了解 EIGRP 的综合度量值及其计算方法。咱们还将了解影响度量值计算，以及调整计算得到的度量值的不同方法。

随后，咱们将了解弥散更新算法 (DUAL) ，以及 EIGRP 的拓扑数据表。这一小节以对在运行 EIGRP 的路由器上产生 IP 路由数据表时，所有这些信息如何整合在一起的讨论结束。


## EIGRP 的综合度量值计算

增强型 IGRP 使用一种复合的度量值，其中包含着一些称为 `K` 值的不同变量。所谓 `K` 值，是一些分配权重到路径不同方面的常数，这些方面会包含在这一综合 EIGRP 度量值中。这些 `K` 值的默认值，为 `K1 = K3 = 1` 及 `K2 = K4 = K5 = 0`。 换句话说，`K1` 与 `K3` 被设置为 1 的默认值，而 `K2`、`K4` 和 `K5` 被设置为 0 的默认值。
...
这基本上意味着，默认情况下，EIGRP 使用了到目的网络路径上的最小带宽与总累积延迟，计算路由度量值。不过，Cisco I0S 软件允许管理员，设置别的 K 值为一些非零值，以便将其他变量纳入这个综合度量值。这可使用 `metric weights [tos] k1 k2 k3 k4 k5` 这条路由器配置命令完成。
...
增强型 IGRP 的度量值计算，可通过使用 `bandwidth` 命令，调整指派给各个接口的默认带宽值，而予以直接影响。由这条命令指定的带宽值，以千位（kilobits）为单位。而用于 EIGRP 度量值计算的带宽，也是以千位计算。下图 22.5 演示了一个由通过两条有着 1544Kbps 带宽值的串行 (T1) 链路连接的两台路由器，所构成的网络。


![EIGRP度量值的带宽修改](../images/3608.png)

**图 22.8** -- **EIGRP 度量值的带宽操纵**

参照图 22.8 中的图示，由于 `R1` 和 `R2` 之间两条链路相等的带宽（及延迟）值，因此从 `R2` 到 `172.16.100.0/24` 这个子网的两条路径的同一 EIGRP 度量值就将被推导出来。EIGRP 将在这两条串行链路之间负载分担流量，如 `R2` 上的以下输出所示：
...
- `Serial0/0` 上的带宽值可被提升，得到这一路径的一个更好（更低）度量值；
- `Serial0/1` 上的带宽值可被降低，得到该路径的一个更差（更高）度量值。


两个选项中任一个都是可以接受的，并都将达到所要的结果。以下输出演示了如何降低 `Serial0/1` 上的默认带宽值，从而有效确保 `Serial0/0` 被用作 `R2` 与 `172.16.100.0/24` 这个网络之间的主路径：
...
**注意**：这一配置并不意味着 `Serial0/1` 现在只具备通过此接口的 1024Kbps 的吞吐量能力。

这一配置的结果即为，`Serial0/0` 是 `R2` 用于到达 `172.16.100.0/24` 这个目的网络的主要路径。这在以下输出中得以演示：
...
虽然经由 `Serial0/1` 接口的那条路径不会被安装到路由数据表中，但在使用 EIGRP 作为路由协议时，重要的是记住，这条路径不会被完全忽略。相反，这条路径被存储在 EIGRP 的拓扑数据表中，其包含着到远端目的网络的主路径和备用（备份）路径。EIGRP 的拓扑数据表，将在这一教学模组稍后详细介绍。

**注意**：默认情况下，在 EIGRP 被启用后，他可能使用接口带宽的最多 50% 发送 EIGRP 数据包（EIGRP 是种非常健谈的协议，因此他在可能的带宽使用量方面对自己有所限制）。EIGRP 会根据 `bandwidth` 这条接口配置命令，确定出带宽量。因此，在调整接口带宽值时，记住这点就很重要。这一默认设置，可通过使用 `ip bandwidth-percent eigrp [ASN] [percentage]` 这条接口配置命令调整。
...
经由 `Serial0/1` 的那条路径，作为到该网络的一条备用路径保留在拓扑数据表中，如下所示。


```console
R2#sh ip eigrp topology
IP-EIGRP Topology Table for AS(150)/ID(150.2.2.2)
Codes: P - Passive, A - Active, U - Update, Q - Query, R - Reply,
       r - reply Status, s - sia Status

P 192.168.122.0/24, 1 successors, FD is 1686016
        via 150.1.1.1 (1686016/28160), Serial1/0
        via 150.2.2.1 (2172416/28160), Serial1/1
P 150.2.0.0/16, 1 successors, FD is 2169856
        via Summary (2169856/0), Null0
P 150.2.2.2/32, 0 successors, FD is Inaccessible
        via 150.1.1.1 (2195456/2169856), Serial1/0
        via 150.2.2.1 (2681856/2169856), Serial1/1
P 150.1.1.2/32, 0 successors, FD is Inaccessible
        via 150.1.1.1 (2195456/2169856), Serial1/0
        via 150.2.2.1 (2681856/2169856), Serial1/1
P 150.1.1.0/24, 1 successors, FD is 1683456
        via Connected, Serial1/0
P 150.2.2.0/24, 1 successors, FD is 2169856
        via Connected, Serial1/1
P 150.1.0.0/16, 1 successors, FD is 1683456
        via Summary (1683456/0), Null0
P 150.1.1.1/32, 1 successors, FD is 2169856
        via Rconnected (2169856/0)
P 150.2.2.1/32, 1 successors, FD is 2169856
        via Rconnected (2169856/0)
...
所谓弥散更新算法（DUAL），是 EIGRP 路由协议的关键。DUAL 会查看接收自邻居路由器的所有路由，对他们加以比较，然后选出到目的网络的最低度量值（最佳）无环路路径，即可行距离 (Feasible Distance, FD)，从而得到后续路由。FD 同时包含由所连接邻居通告的网络度量值，以及到达该特定邻居的开销。

邻居路由器通告的度量值，被称为到目的网络报告的距离（Reported Distance, RD），或通告的距离（Advertised Distance, AD）。因此，FD 就包括了 RD，加上到达该特定邻居的开销。这条后续路由的下一跳路由器，称为接续者。接续者的路由，会被置于 IP 路由数据表以及 EIGRP 拓扑数据表中，并指向该接续者。

有着比后续路由 FD 更低 RD 的，到同一目的网络的任何其他路由，都保证是无环路的，并称为可行后续（Feasible Successor, FS）路由。这些路由不会被置于 IP 路由表中；但他们仍会与后续路由一道，置于 EIGRP 的拓扑数据表中。

为了使某条路由成为 FS 路由，其必须满足可行性条件（Feasible Condition, FC），只有在到目的网络的 RD 小于 FD 时，可行性条件才会出现。在 RD 大于 FD 的情形下，那么该路由就不会被选为 FS。这一条件被 EIGRP 用于防止环路的可能。下图 22.10 中所示的网络拓扑结构，将用于阐明这一小节中提到的术语。
...
根据表 22.4 中的信息，`R1` 将根据该路由 25 的 FD，选择通过 `R4` 的路径作为后续路由。这条路由将被置于 IP 路由表及 EIGRP 的拓扑数据表中。`R1` 随后会查看到 `192.168.100.0/24` 网络的那些备用路径。邻居 `R3` 到 `192.168.100.0/24` 网络的度量值，也称为 RD 或 AD，为 10。这小于 FD。那么这条路由满足 FC，而会被置于 EIGRP 的拓扑数据表中。邻居 `R2` 到 `192.168.100.0/24` 网络的度量值为 30。这一值高于 25 的 FD。那么这条路由就不满足 FC，而不会被视为一条 FS。但是，这条路由仍会被置于 EIGRP 的拓扑数据表中。这点在接下来的 EIGRP 拓扑数据表小节中会得以演示。

当某个邻居更改某一度量值，或某种拓扑变化发生，进而后续路由被移除或变更时，DUAL 就会检查该路由的那些 FS。当某一 FS 找到时，DUAL 就会使用他，以避免不必要地重新计算路由。这种做法称为本地计算。执行一次本地计算，节省了 CPU 算力，因为在后续路由或主路由失效前，FS 就已被选中并已经存在。

当没有目的网络的 FS 存在时，那么本地路由器将发送一个查询数据包到邻近路由器，询问他们是否有着关于该目的网络的信息。当该信息可用，且另一邻居确实有着到这个目的地网络的一条路由时，那么这个路由器就会执行一次弥散计算，以确定出新的接续者。
...
EIGRP 的拓扑数据表，由 DUAL 有限状态机作用下的各个 EIGRP 协议相关模组（PMD）产生。由相邻 EIGRP 路由器通告的全部已知目的网络与子网，均存储在 EIGRP 拓扑数据表中。这包括了后续路由、FS 路由，甚至那些未满足 FC 的路由。

拓扑数据表允许所有 EIGRP 路由器，都有着对整个网络的一致视图。他还实现了 EIGRP 网络的快速收敛。拓扑数据表中的各个单独条目，都包含目的网络，以及通告该目的网络的邻居。FD 与 RD 二者都存储在这个拓扑数据表中。EIGRP 的拓扑数据表，包含了建立到每个可达网络的距离与矢量集所需的信息，包括以下这些：
...
**注意**：虽然 MTU 包含在拓扑数据表中，但 EIGRP 不会在实际度量值计算中使用这个值。相反，MTU 只会因确定出到目的网络的最小值而被跟踪。接口的 MTU 指定了在无需分片/分段，或将数据报或数据包分割成小块下，即可在某一特定链路上传输的最大数据报大小。
...
`show ip eigrp topology [network]/[prefix]` 与 `show ip eigrp topology [network] [mask]` 两条命令，会打印在两条命令中所指定路由的后续路由、FS 路由及那些未满足 FC 的路由。以下输出演示了 `show ip eigrp topology [network]/[prefix]` 这条命令的用法：
...
在上面的输出中，咱们可以看到经由 `Serial0/1` 的那条路径，就不满足 FC，因为 RD 超过了 FD。这就是为什么该路径未在 `show ip eigrp topology` 命令的输出中被打印的原因。咱们可使用 `show ip eigrp topology all-links` 命令，查看 EIGRP 拓扑数据表中，所有前缀的所有可能路由，而不是逐个地查看每个前缀，确定后续路由、FS 路由，以及那些不满足 FC 的路由。这一命令的输出如下所示。
...
在 EIGRP 的拓扑数据表中，条目可能会被标记为被动 (P) 或主动 (A)。处于 “被动” 状态的某条路由，表明 EIGRP 已完成该条路由度量值的主动计算，流量可通过使用该后续路由，转发到目的网络。这是拓扑数据表中所有路由的首选状态。

当后续路由丢失，路由器发出一个查询数据包以确定某一 FS 时，那么这些增强型 IGRP 路由便处于主动状态。通常，会由某一 FS 存在，而 EIGRP 会将其提升为后续路由。在这种方式下，路由器会在不涉及网络中其他路由器下收敛。这一过程被称为本地计算。

但是，当后续路由丢失或被移除，且没有 FS 时，那么路由器将开始弥散计算。在弥散计算下，EIGRP 将发出一个查询数据包到所有邻居，并在除了到后续路由接口外的全部接口上发出。当某一 EIGRP 邻居收到某条路由的查询数据包时，若该邻居的 EIGRP 拓扑数据表中不包含该路由的条目，那么该邻居会立即以一条不可达报文，回复这个查询数据包，声明没有经由这一邻居的这条路由的路径。

当邻居上的 EIGRP 拓扑数据表，将发送这个查询的路由器列为该路由的接续者，并且一个 FS 存在时，那么这个 FS 将被安装，同时该路由器将回复这次邻居查询，他有着一条到那个丢失目的网络的路由。

> 译注：即该邻居原先有条经由发出查询路由器到目的网络的 FD，又有条经由其他路由器到目的网络的 FS，此时原先的 FD 已不可用，因此原先的 FS 将成为新的 FD。

但是，当 EIGRP 的拓扑数据表，将发送查询的路由器列为此路由的接续者，且没有 FS 时，那么该路由器就会查询其所有 EIGRP 邻居，除了与那其先前接续者的同一接口发出的路由器外。在其收到他为这条路由发起的所有查询的一次回复前，该路由器不会回复这个查询数据包。
...
最后，当查询在某个并非该目的地接续者的邻居上收到的时，那么该路由器会回复以自己的接续者信息。当这些相邻路由器都没有那条丢失路由的信息时，那么一些查询数据包，便会自这些相邻路由器，发送到其相邻路由器，直到到达查询边界（the Query boundary）。所谓查询边界，可以是网络末端、分发列表的边界，或者汇总的边界。

一旦查询数据包已发出，EIGRP 路由器在其计算后续路由前，就必须等待收到所有的回复数据包。当所有邻居在三分钟内均未回复时，那么这条路由就被指为卡在主动状态（SIA）。当某条路由处于 SIA 状态时，未回复这一查询的路由器邻居关系，将被重置。在这种情形下，咱们将看到一条类似下面的被路由器日志记录的消息：
...
EIGRP 邻居路由器未响应查询数据包的原因可能有数种，其中包括下面这些：

- 邻居路由器的 CPU 过载，进而其无法及时响应
- 邻居路由器本身没有关于这条丢失路由的信息
- 电路质量问题，正导致数据包丢失
- 某些低带宽链路经历拥塞，进而数据包正被延迟


要防止因其他 EIGRP 邻居的延迟响应而导致的 SIA 问题，可通过在路由器配置模式下，使用 `timers active-time` 命令，将本地路由器配置为等待长于默认的三分钟，接收对其查询数据包的响应。
