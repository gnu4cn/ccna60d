...
在上面的输出中，`150` 便是 ASN。当未对任何 `show ip eigrp` 命令指定某个自治系统时，那么默认在 Cisco IOS 下，会打印所有 EIGRP 实例的信息。
...
那么当 EIGRP 被启用以供使用，且主有类的 `10.0.0.0/8` 网络与 `network router` 配置命令结合使用时，则全部四个 `Loopback` 接口都会针对 EIGRP 路由被启动。这在以下输出种得以演示：
...
**注意**：拓扑数据表、EIGRP `Hello` 数据包及 `Update`报文，会在这个教学模组稍后详细介绍。这一小节的重点，仅限于 EIGRP 的配置实施。
...
如上面的配置中所示，在 Cisco 10S 软件中，使用通配符掩码或子网掩码，都会导致同样的操作，与 `network` 语句配置。
