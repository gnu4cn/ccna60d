...
与会发送包含全部路由信息的定期路由更新，到其邻居的传统距离矢量路由协议不同，EIGRP 会发送一些非定期的增量路由更新，在整个路由域中分发路由信息。EIGRP 的增量更新，会在在网络拓扑发生变化时发送。
...
增强型 IGRP 使用两种独特的类型/长度/值（TLV）三元组，承载路由条目。这两种 TLV 分别是内部的 EIGRP 路由 TLV，以及外部的 EIGRP 路由 TLV，分别用于内部及外部的 EIGRP 路由。这两种 TLV 都包含一个指定了用于目标网络子网掩码位数的 8 位前缀长度字段。包含于这个字段中的信息，允许 EIGRP 能够支持不同子网划分的网络。
...
为了确保网络中无环路的路径，EIGRP 使用了用于跟踪由邻居通告的所有路由的弥散更新算法 (DUAL)，然后选出到目标网络的最佳无环路路径。DUAL 是一项将在这个教学模组稍后介绍的 EIGRP 核心概念。
