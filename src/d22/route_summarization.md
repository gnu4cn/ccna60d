...
路由汇总减少了路由器必须处理的信息量，实现了网络内的更快速收敛。汇总还通过隐藏网络内某些区域的详细拓扑信息，限制受网络变化影响的区域大小。最后，正如这一教学模组前面所指出的，汇总用于定义 [EIGRP 的查询边界](./metrics_dual_and_topology_table.md#query-boundary)，EIGRP 支持两种路由汇总类型，如下所示：
...
默认情况下，当 EIGRP 于路由器上启用时，自动路由汇总便生效了。这是使用 `auto-summary` 命令实现的。这条命令允许 EIGRP 在有类边界处，执行自动路由汇总。这一默认特性的运行，通过使用下图 22.15 中的网络拓扑得以演示。
...
参考图 22.15 中所示的 EIGRP 网络，`R1` 和 `R2` 正运行着 EIGRP，并使用着自治系统 150。子网 `10.1.1.0/24`、`10.2.2.0/24` 及 `10.3.3.0/24` 均直连到 `R1`，`R1` 正将这些路由通告到 `R2`。`R1` 和 `R2` 通过 `150.1.1.0/24` 子网（其是个不同于 `10.1.1.0/24`、`10.2.2.0/24` 和 `10.3.3.0/24` 子网的主网络）上的背靠背串行连接相连。根据连接到这两个路由器的网络，默认情况下 EIGRP 将执行自动汇总，如下所示：
...
在上面的输出中，子网 `10.1.1.0/24`、`10.2.2.0/24` 和 `10.3.3.0/24`，已被自动汇总为 `10.0.0.0/8`。这个摘要地址，会从 `Serial0/0` 通告出去。子网 `150.1.1.0/24` 已被汇总为 `150.1.0.0/16`。这个摘要地址会从 `Loopback1`、`Loopback2` 及 `Loopback3` 通告出去。请记住，默认情况下，EIGRP 将在 EIGRP 路由被启用的所有接口上发出更新。


## 被动接口

<a name="passive-interface"></a>
参考上面打印的输出，咱们可以看到，在某个 `Loopback` 接口上发送更新属于一种资源浪费，因为设备无法物理连接到路由器的 `Loopback` 接口以监听此类更新。这种默认行为，可通过使用 `passive-interface` 这条路由器配置命令予以禁用，如下所示：
...
这一配置的结果，便是 EIGRP 数据包不再从环回接口发出。因此，如下所示，摘要地址便不会从这些接口发出了。
...
IP-EIGRP Topology Table for AS(150)/ID(10.3.3.254)
...
P 150.1.1.2/32, 1 successors, FD is 2169856
        via Rconnected (2169856/0)
P 150.1.1.0/30, 1 successors, FD is 2169856
        via Connected, Serial1/0
P 150.1.1.1/32, 0 successors, FD is Inaccessible
        via 150.1.1.2 (2681856/2169856), Serial1/0
```

在路由表中，这条摘要路由是直连到 `NullO` 接口的。这条路由有着默认 5 的管理距离值。这在以下输出中得以演示：
...
当 EIGRP 执行自动汇总时，路由器会通告摘要路由，并抑制那些更具体路由。换句话说，在摘要路由被通告的同时，那些更具体路由会在到 EIGRP 邻居的更新中被抑制。如下所示，这点可通过查看 `R2` 上的路由表加以验证。
...
<a name="discontiguous-network"></a>
这种默认行为在诸如上 [图 22.12](#f-22.15) 中所示的一些基本网络中工作良好。但是，他会对不连续网络有着不利影响，所谓不连续网络，由分开了另一主网络的某个主网络构成，如下图 22.16 中所示。
...
参照图 22.16 中所示的图示，其中的 `150.1.0.0/16` 主网络，就分开了两个 `10.0.0.0/8` 的主网络。在自动汇总启用时，`R1` 和 `R2` 将把子网 `10.1.1.0/24` 和 `10.2.2.0/24`，都汇总为 `10.0.0.0/8` 这个地址。这条汇总路由，将以下一跳接口 `Null0` 被安装。`Null0` 接口是个 “二进制位的垃圾桶”。任何发送到这个接口的数据包都会被丢弃。

由于两台路由器都只会通告给对方摘要地址，因此两台路由器都将无法到达另一路由器的 `10.x.x.x/24` 子网。要理解图 22.16 所示网络中的自动汇总影响，我们从 `R1` 和 `R2` 上的配置开始逐步分析，他们的配置如下：
...
由于有类边界处的自动汇总，在两台路由器上都被默认启用，因此两台路由器均都将生成两个汇总地址：一个是 `10.0.0.0/8`，另一个是 `150.1.0.0/16`。这两个摘要地址都将指向 `NullO` 接口，同时 `R1` 上的路由表将显示以下条目：
...
但是，由于没有具体路由条目存在，因此发送到主 `150.1.0.0/16` 网络的任何其他子网数据包，都将被发送到 `Null0` 这个接口。

到目前为之，一切似乎都井然有序。咱们可以看到，由于主 `150.1.0.0/16` 网络的更具体路由条目，`R1` 和 `R2` 可以互相 `ping` 通。但问题在于 `R1` 和 `R2` 上的那些主 `10.0.0.0/8` 网络子网之间的连通性。路由器 `R1` 显示了其生成的 `10.0.0.0/8` 这个摘要地址的以下具体路由条目：
...
同样，路由器 `R2` 显示了其生成的 `10.0.0.0/8` 摘要地址的以下具体条目：
...
第一种选项非常基础。但是，静态路由的配置不具扩展性，而在大型网络中需要大量配置开销。第二个选项，也是推荐的选项，既具备扩展性，同时相比第一种需要更少的配置开销。如下所示，自动汇总是通过执行 `no auto-summary` 命令禁用的（在较新的 IOS 版本中，其默认已禁用）：
...
这一配置的结果，便是主网络的那些具体路由会由两个路由器同时通告。摘要路由不会得以生成，如下所示。
...
在我们提及手动的路由汇总（不在考试大纲中）前，重要的是要清楚，EIGRP 不会自动汇总外部网络，除非某个内部网络将包含在摘要中。要更好地地理解这一概念，请参阅下图 22.17，其演示了个基本的 EIGRP 网络。
...
参考图 22.17，`R1` 正在重分发（这使他们成为外部网络），并随后经由 EIGRP 通告 `10.0.0.0/24`、`10.1.1.0/24`、`10.2.2.0/24` 和 `10.3.3.0/24` 这些外部网络。自动路由汇总在 `R1` 上是启用的。`R1` 上的初始配置如下：
...
正如前一示例所示，由于这些 `10.x.x.x/24` 前缀均为外部路由，EIGRP 将不会自动汇总这些前缀。因此，EIGRP 将既不会添加一条摘要路由到拓扑数据表，也不会添加到 IP 路由表中。这在以下输出中得以演示：
...
如下所示，这些具体路由条目会作为外部 EIGRP 路由通告给 `R2`：
...
现在，假设子网 `10.0.0.0/24` 是个内部网络，而子网 `10.1.1.0/24`、`10.2.2.0/24` 和 `10.3.3.0/24` 均为一些外部路由。由于将要构成有类摘要地址 `10.0.0.0/8` 的这些路由之一，是条内部路由，因此 EIGRP 就将创建一个摘要地址，并将其包含在 EIGRP 的拓扑数据表及 IP 路由表中。`show ip protocols` 命令显示，网络 `10.0.0.0/24` 现在是个内部的 EIGRP 网络，如下所示。（译注：原文如此，不过这里疑似有误，`10.0.0.0/8` 现在是个 EIGRP 内部网络。）
...
在上面的输出中，由于 `10.0.0.0/24` 这个内部子网，属于聚合地址的一部分，因此 EIGRP 的自动汇总已生成了 `10.0.0.0/8` 的摘要地址。EIGRP 的拓扑数据表，显示了外部与内部的那些条目，以及这个摘要地址，如下所示。
...
从 `R2` 的角度看，这只是一条内部的 EIGRP 路由。换句话说，该路由器并不知道，这个摘要地址也由外部路由构成，如下所示。
...
如下所示，经由所接收到的摘要路由，`R2` 能够同时到达内部的 `10.0.0.0/24` 网络，以及别的外部 `10.x.x.x/24` 网络。
...
不同于 EIGRP 的自动汇总，EIGRP 的手动路由汇总，是通过使用 `ip summary-address eigrp [ASN] [network] [mask] [distance] [leak-map <name>]` 这条接口配置命令，在接口级别配置与实施的。默认情况下，EIGRP 的摘要地址，会指派以一个 5 的默认管理距离值。这种默认指派，可通过指定由 `[distance]` 关键字所指定的期望管理距离值加以修改。

（由于手动摘要不在 CCNA 考试大纲中，我们将不再详述。）

> *译注*：以下内容为早期版本中的内容，已在第 4 版中移除。

默认情况下，在手动的路由汇总配置后，EIGRP 将不通告那些属于汇总网络条目中的那些更具体路由条目。`[leak-map <name>]` 关键字可被配置为允许 EIGRP 的路由泄漏，其间 EIGRP 会允许一些指定的具体路由条目，与摘要地址一起得以通告。那些未在泄漏映射中指定的条目，仍将被抑制。

在手动汇总路由时，重要的是要尽可能具体。否则，配置便可能以早先曾描述过的 [不连续网络](#discontiguous-network) 相关示例类似的方式，导致流量黑洞。这一概念在下图 22.18 中得以演示。
...
参考图 22.18，当一个 `10.0.0.0/8` 的手动摘要地址，于两台路由器上同时配置了时，那么那些更具体前缀即被抑制。由于 EIGRP 还会以 `Null0` 的下一跳接口，同时安装一条到这个摘要地址的路由到 EIGRP 拓扑数据表及 IP 路由表中，因此在不连续网络中自动摘要下曾遇到过的同一问题，在这个网络中也会遇到，而两台路由器上的相应子网，都将无法相互通信。


## 不良路由汇总导致的次优路由问题

此外，重要的是要理解，当在网络中部署不良时，路由汇总会导致网络中的次优路由。这一概念在下图 22.19 中得以演示：

![路由汇总下的次优路由问题](../images/3619.png)

**图 22.19** -- **路由汇总下的次优路由**

默认情况下，在 EIGRP 的一条摘要路由被创建出后，路由器会以等于所有更具体路由中最小的度量值，通告这个摘要地址。换句话说，这个摘要地址将有着与包含在这个摘要地址的创建中，最低、最具体路由的同一度量值。
...
根据表 22.5 中的度量值计算，对于源自 `R1` 的流量，`R2` 无疑有着到 `10.4.4.0/24` 的最低度量值路径，而对于源自 `R1` 的流量，`R3` 则有着到 `10.6.6.0/24` 的最低度量值路径。但是，当 `10.0.0.0/8` 这个摘要地址被通告到 `R1` 时，摘要地址会使用构成这一摘要的所有路由中，最低的最小度量值。根据这一示例，`R2` 就会以 10 的度量值，通告摘要地址到 `R1`。`R3` 会遵循同一逻辑，以 15 的度量值通告这条摘要路由到 `R1`。

当 `R1` 收到来自 `R2` 和 `R3` 的两条摘要路由时，他会使用有着最低度量值的那条，经由 `R2` 转发以包含于 `10.0.0.0/8` 主有类网络中的那些子网为目的地的流量。这在下图 22.20 中得以演示：
...
回到使用 EIGRP 时的手动路由汇总配置，下图 22.21 中所示的网络拓扑，将用于演示手动路由汇总与路由泄漏：
...
要汇总 `R1` 上的这些条目并通告单条具体路由，那么以下配置就要应用到 `R1` 的 `Serial0/0` 接口：
...
路由表条目也反映了这条以 `Null0` 为下一跳接口的摘要路由，如下所示：
...
在 `R2` 上，`10.0.0.0/14` 这个摘要地址的单条路由条目会被收到，如下所示：
...
为强化关于摘要路由度量值这一概念，假设 `R1` 上的那些路由，都是一些有着不同度量值的外部 EIGRP 路由（即，他们已被重分发到 EIGRP 中）。那么 `R1` 上的 EIGRP 拓扑数据表，就会显示以下内容：
...
基于这一配置，这条摘要路由就会以等于其包含的所有路由的最低度量值，被置于 EIGRP 的拓扑数据表与 IP 路由表中。根据早先给出的 `show ip eigrp topology` 命令的输出，这个汇总地址将被分配与 `10.3.3.0/24` 这个前缀所分配的同一度量值，如下所示：
