...
路由汇总可减少路由器必须处理的信息量，从而加快网络内的收敛速度。路由汇总还可以隐藏网络中某些区域的详细拓扑信息，从而限制受网络变化影响的区域大小。最后，如本模块前面所述，汇总用于定义 EIGRP 的查询边界，它支持以下两种路由汇总：

自动路由汇总
手动路由汇总


默认情况下，当路由器启用 EIGRP 时，自动路由汇总将生效。这是通过 auto-summary 命令实现的。该命令允许 EIGRP 在类边界执行自动路由汇总。下图 22.12 中的网络拓扑说明了这一默认功能的操作。
