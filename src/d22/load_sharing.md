...
`maximum-paths <1-6>` 这条路由器配置命令，可用于将默认四条的最大路径数，修改为最多六条等价路径。在执行等价的负载均衡时，路由器会在所有路径间，平均分配负载。流量份额计数，表明了各条路径上的出站数据包数量。在执行等价的负载均衡时，一个数据包是在每单条路径上发送的，如以下输出中所示：
...
除了等价的负载均衡能力外，EIGRP 还能执行非等价的负载分担。这一独特功能，允许 EIGRP 根据加权流量的分担值，使用一些开销不相等的路径，发送出站数据包到目的网络。非等价的负载分担，是通过使用 `variance <multiplier>` 这条路由器配置命令启用的。


其中 `<multiplier>` 这个关键字，是个介于 1 与 128 之间的整数。默认情况下，1 的倍数，意味着没有非等价的负载分担被执行。这一默认设置，下面的 `show ip protocols` 命令输出中得以演示。
...
所谓倍数，是个指示路由器，在一些有着低于那个最小度量值乘以倍数后度量值的路由上，负载分担的可变整数。例如，当指定了 5 一个方差时，就会指示路由器，在那些度量值小于 5 倍最小度量值的路由上负载分担。默认 1 的方差，则告诉路由器，执行等价的负载均衡。在这个 `variance` 命令被用到，同时某个 1 以外的值被指定作为倍数时，路由器就将以每条单独路由的度量值，按比例在这些路由之间分配流量。换句话说，相比那些有着较高度量值的路径，路由器将以有着较低度量值的路径发送更多流量。
...
其中的流量共享计数表明，对于每 60 个经由 `Serial0/0` 转发的数据包，路由器将经由 `Serial0/1` 转发 47 个数据包。这是在考虑两条路径的路由度量值下，按比例完成的。这是 `variance` 命令部署后的默认行为。这种智能的流量分担功能，时经由 `traffic-share balanced` 这条路由器配置命令启用的，而其无需显式配置。

**注意**：默认情况下 `traffic-share balanced` 这条命令是启用的，且即使手动配置了也不会出现在运行配置中。这点在下面得以演示。
...
正如这一小节前面所指出的，在 `variance` 命令被用到时，所有满足可行性条件，且有着小于最小度量值乘以倍数的路径，都将被安装到路由表中。该路由器随后将使用所有路径，并根据路由度量按比例负载分担流量。

在一些情形下，咱们会打算允许一些诸如可行接续者路由等替代路由被置于路由表中而不被使用，除非接续者路由被移除。在启用 EIGRP 的网络中，此类操作通常是要缩短收敛时间而执行的。要理解这一概念，请回想一下，默认情况下路由器只会将接续者路由，置于 IP 路由表中。当该接续者路由不再可用时，可行接续者（FS）路由便会被被提升为接续者路由。这条路由随后便会作为到目的网络的主要路径，被安装到路由表中。

`traffic-share min across-interfaces` 这条路由器配置命令，可与 `variance` 命令结合使用，将那些有着小于最小指标乘以倍数的所有路由，都安装到路由表中，而仅使用有着最小（最佳）度量值的路由转发数据包，直到该路由不可用为止。这种配置的主要目的，是在这条主路由丢失的情况下，替代路由已在路由表中，并可被立即使用。

以下配置示例，使用了上 [图 22.11](#f-22.11) 中所示的拓扑结构，演示如何将路由器配置为，把那些有着小于两倍最小度量值的路由置于路由表中，而只使用有着最小度量值的那条路由实际转发数据包：
...
如上面的输出中所示，根据这种方差配置，两条不同度量值的路由，已被安装到路由表中。但是，请注意，经由 `Serial0/1` 的那条路由的流量分担计数为 0，而经由 `Serial0/0` 的那条路由的流量分担计数为 1。这意味着即使经由 `Serial0/1` 的路由条目已安装到路由表中，该路由器也将不经由 `Serial0/1` 发送任何数据包到 `172.16.100.0/24`，直到经由 `Serial0/0` 的那条路径不再可用。
