...
这一小节介绍了 EIGRP 用到的不同类型报文。不过，在深入探讨这些不同报文类型的具体内容前，必须扎实了解包含这些报文的 EIGRP 数据包头部。
...
下图 22.2 演示了不同字段，在其出现与某次 EIGRP 数据包的线路捕获中时的样子：
...
在这个 EIGRP 数据包头部中，4 位的 `OPCode` 字段，用于指明该 EIGRP 数据包的类型或报文。EIGRP 使用不同的报文或数据包类型，包括 `Hello` 数据包、确认数据包、更新数据包、查询数据包、回复数据包及请求数据包。这些数据包类型，在接下来的小节中得以介绍。
