...
OSPF 度量值的计算，可通过执行以下操作而直接予以影响：

- 使用 `bandwidth` 命令调整接口带宽
- 使用 `ip ospf cost` 命令手动指定开销


`bandwidth` 命令的用法，曾在 [早先的一个教学模组](../d22/metrics_dual_and_topology_table.html#运用接口带宽影响-eigrp-的度量值计算) 中，我们讨论 `EIGRP` 的度量值计算时讨论过。正如前面曾指出的，默认的 OSPF 开销，是通过将 10<sup>8</sup>， 或 100 Mbps 的参考带宽，除以链路带宽计算出的。增加或减少链路带宽，都会直接影响特定链路的 OSPF 开销。这通常属于一种路径控制机制，用于确保一条路径优先于另一路径。
...
`ip ospf cost <1-65535>` 这条接口配置命令，用于手动指定某一链路的开销。这个值越低，那么该链路优先于其他到同一目标网络，却有着更高开销值链路的可能性就越大。以下示例演示了针对一条串行 (`T1`) 链路，配置 5 的 OSPF 开销：
