...
再次强调，开放最短路径优先（Open Shortest Path First），是一种通告其链路状态的，开放标准的链路状态路由协议。当某个链路状态路由器开始在一条网络链路上运行时，那么与这个逻辑网络相关的信息，就会被添加到其本地的链路状态数据库（LSDB）中。这个本地路由器，随后就会在其运行的链路上，发送 `Hello` 报文，以确定是否有其他链路状态路由器，也在接口上运行。OSPF 使用 IP 编号 80，直接运行在互联网协议上。


虽然深入研究所有潜在 OSPF 问题情况不大可能，但接下来的小节讨论了，在将 OSPF 作为首选 IGP 部署时，一些最常见的问题场景。
...
OSPF 的邻居数据表可能为空的原因（即 为何 `show ip ospf neighbor` 命令的输出为产生任何结果）有数种。一些常见原因如下：
...
所谓基本的 OSPF 配置错误，包括很多方面。例如，这些可能包括不匹配的计时器、区域 ID、认证参数，以及末梢配置等。大量排除基本 OSPF 配置错误的工具，在Cisco IOS 软件中可用。例如，咱们可使用 `show ip protocols` 这条命令，确定出一些信息（如 有关那些启用 OSPF 的网络）；`show ip ospf` 这条命令，确定区域配置及每个区域的那些接口；而 `show ip ospf interface brief` 这条命令，确定出哪些接口位于哪个区域，以及这些接口启用了哪些 OSPF 进程 ID，假设已针对接口 OSPF 已被启用时。


另一种常见的错误配置，是正将接口指定为被动接口。当情况如此时，那么该接口将不发送 `Hello` 数据包，经而邻居关系也将不通过使用该接口建立。咱们可使用 `show ip protocols` 或 `show ip ospf interface` 命令，检查哪些接口已被配置或指定为被动接口。下面是后一命令在某个被动接口上的示例输出：
...
最后，当在比如帧中继等一些 NBMA 技术上启用 OSPF 时，就要记住，邻居必须被静态配置，因为针对默认的非广播网络类型，OSPF 不会将组播传输用于邻居发现。这是在部署 OSPF 时，空邻居数据表的一种常见原因。我们将在 [WAN 小节](#unknown) 讨论 NBMA。


一层及二层故障，也会导致没有 OSPF 邻居关系形成。一层与二层故障排除，已在前面的教学模组中详细介绍过。要使用诸如 `show interfaces` 等命令，检查接口状态（即线路协议），以及接口上任何接收到的错误。比如当启用 OSPF 的路由器，位于某个跨越多个交换机的 VLAN 中时，纳闷就要验证该 VLAN 内是否有端到端的连通性问题，以及是否所有端口或接口，都处于正确的生成树状态等。


ACL 的过滤，是导致邻接关系建立失败的另一种常见原因。为了排除此类问题，熟悉拓扑结构就非常重要。例如，当建立邻接关系失败的两台路由器，是经由不同物理交换机相连时，那么就可能是出于安全目的，在一些交换机上已配置的 ACL 过滤，正以 VACL 形式实施着。一项可指示出 OSPF 数据包，是否正被拦截或丢弃的有用故障排除工具，便是 `show ip ospf traffic` 这条命令，其会打印如以下输出所示的，有关传输及发送的 OSPF 数据包的信息。
...
空邻居数据表的另一种常见原因，是接口的错误配置。与 EIGRP 类似，OSPF 将不使用辅助接口地址建立邻居关系。但与 EIGRP 不同，当接口的子网掩码不一致时，OSPF 也将不建立邻居关系。

即使接口子网掩码不同，启用 EIGRP 的路由器也会建立邻居关系。例如，但其中一台有着使用 `10.1.1.1/24` 的接口，另一台有着使用 `10.1.1.2/30` 接口的两台路由器，被配置为背靠背的 EIGRP 的部署时，他们将成功建立邻居关系。但应注意的是，这种部署可能会导致这两台路由器之间的路由环路。除了不匹配的子网掩码外，启用 EIGRP 的路由器还会忽略最大传输单元 (MTU) 配置，并在即使两个接口 MTU 值不同的情况下，建立邻居关系。要使用 `show ip interfaces` 及 `show interfaces` 两条命令，验证 IP 地址及掩码配置。
...
OSPF 为通告路由的一种常见原因，便是网络未经由 OSPF 通告。在当前的 Cisco 10S 版本中，网络可通过使用 `network` 这条路由器配置命令，或 `ip ospf` 这条接口配置命令予以通告。无论使用的哪种方法，`show ip protocols` 这条命令都可用于查看 OSPF 配置为要通告的那些网络，正如以下输出种可以看到的那些：
...
此外，要记住，咱们还可以使用 `show ip ospf interfaces` 命令，找出那些已启用 OSPF 的接口等信息。除了网络配置外，当接口宕掉时，OSPF 也将不通告路由。咱们可以使用 `show ip ospf interface` 这条命令，确定出接口的状态，如下所示：
...
参考上面的输出，咱们可以看到， `Loopback100` 便处于 `DOWN` 状态。再仔细观察，咱们就会看出，问题出在该接口已被管理性关闭上，如下输出中所示：
...
除上述三种常见原因外，糟糕的设计、部署，及错误配置，也是 OSPF 可能未按预期通告网络的另一原因。导致此类问题的常见设计问题，包括例如不连续或分区的骨干区域，以及区域类型的错误配置，比如区域配置为了完全末梢区域等等。出于这一原因，扎实了解这种协议的工作原理，以及在咱们的环境中的其已被怎样部署等，就非常重要。这种了解，将大大简化故障排除过程，因为在咱们开始故障或问题的排除前，就已经赢了一半。
