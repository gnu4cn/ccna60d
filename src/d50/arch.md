...
南向的那些 API，会中继信息到路由器与交换机。OpenFlow 是最先投入市场的架构，并是南向 API 的鼻祖。思科则提供了应用中心基础设施（ACI，见下图 50.18）。设备位于控制器之下，同时 API 实现了数据交换。南向的接口允许控制器与网络设备通信。存在不同的控制器类型，每种控制器类型有着不同能力。下面是咱们应留意的三个示例：
...
北向的 那些 API，被 SDN 用于与应用及业务逻辑的通信。北向的接口会放开控制器，允许程序访问数据及功能。
