...
配置 BPDU 由 LAN 交换机发送，用于通信和计算生成树拓扑。交换机端口初始化后，端口会进入阻塞状态，并向交换机的每个端口发送一个 BPDU。默认情况下，在与其他交换机交换配置 BPDU 之前，所有交换机最初都会认为自己是生成树的根节点。只要端口继续认为自己的 "配置 BPDU "最有吸引力，它就会继续发送 "配置 BPDU"。交换机根据以下四个因素（按顺序排列）确定最佳配置 BPDU：
